...
- **Servidor:** AWS Lightsail Ubuntu 24.04, `18.191.129.33` (us-east-2, misma región que el pooler de Supabase), usuario `ynt` (grupo `docker`). Migrado desde el homelab el 2026-08-05.
  ...
  - **Fallback frío:** el homelab (`homelab.casa`, WiFi solo 2.4GHz) conserva el mismo stack con su `cloudflared-main` **y su contenedor Rails detenidos** (2026-08-06: el contenedor había quedado corriendo sin túnel y consumía ~13 de las 15 conexiones del pooler de Supabase — producción en Lightsail daba `EMAXCONNSESSION` bajo carga; por eso el fallback se apaga COMPLETO). **OJO (2026-08-31): el standby NO EXISTE hoy.** El contenedor sincronizado el 2026-08-15 (`…-web-a30140f5…`) murió con el disco del homelab (2026-08-24) y la resurrección no lo recreó — `docker ps -a` de la caja no tiene ningún contenedor de fitness, así que **no hay failover posible hasta re-sincronizar**. La imagen de producción (`28b3f95…`) quedó transferida a la caja el 2026-08-31; falta el contenedor, y el §4 necesita una decisión: el script clona los secretos DEL CONTENEDOR VIEJO, que ya no está — la provisión del env es del dueño (mismo dilema que el standby de NomiCheck resolvió con el camino directo prod→homelab, sin pasar por la Mac). Cuando vuelva, el failover será `ssh ynt@homelab.casa "docker start advance_fitness_app-web-<sha-sincronizado> && docker start cloudflared-main"` — actualizar el sha acá al re-sincronizar — (y detener el cloudflared **y el contenedor** de Lightsail: repartir tráfico entre versiones ya causó el bug del tenant perdido, y ambos contenedores arriba re-saturan el pooler).
