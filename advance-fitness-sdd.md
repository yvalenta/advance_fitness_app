...
| 20 | Ideas de openGym que tocan el contrato de rutina (agosto 2026 — decisiones en Nota 27) | ~1 semana | Las cinco ideas de la Fase 19 que quedaron pendientes por tocar `CAMPOS_EJERCICIO`/uid/mesociclo: **reps por lado** (`unilateral`, división de lectura vía `reps_por_lado`, el total se registra igual); **ejercicios por tiempo** (`tipo: "tiempo"` reusa `repeticiones` como segundos; timer de trabajo en el modo sesión que registra los segundos REALES sostenidos, no siempre el objetivo); **superseries** (`grupo_superserie`: dos ejercicios con el mismo valor se alternan serie a serie sin descanso entre sí, descanso solo al cerrar la ronda); **progresión algorítmica** (`Progresion::Regla`, ajusta `peso_sugerido_kg` semana a semana por reglas, sin IA); **push del rest-timer** con la app cerrada (reutiliza `Notificaciones::EnviadorPush`, opt-in aparte del de racha). Ninguno lo escribe la IA — son ajuste manual del staff sobre un plan ya generado (Nota 27) | El staff marca un ejercicio unilateral y el miembro ve "por lado" en su rutina; una plancha se cronometra sola y registra el tiempo real sostenido; dos ejercicios agrupados se alternan sin descanso entre sí; el peso sugerido sube solo cuando el miembro cumple el objetivo de reps; el rest-timer avisa aunque el miembro haya cerrado la app |
...
>
> **Nota 27 (agosto 2026) — Fase 20, las cinco ideas de openGym que tocaban el contrato de rutina:** segunda tanda de la Nota 26, esta vez sobre `CAMPOS_EJERCICIO`/uid/mesociclo/editor de staff. Tres campos nuevos en `PlanPersonalizado::CAMPOS_EJERCICIO` (`unilateral`, `tipo`, `grupo_superserie`) — **ninguno lo escribe la IA** (ni el prompt ni `Ejercicios::ValidadorRutina` los conocen): son ajuste manual del staff sobre un plan ya generado, coherente con mantener el catálogo/contrato de IA cerrado (Fase 6.5) sin ampliar su superficie. **(a) Reps por lado** — `unilateral: true` + `ApplicationHelper#reps_por_lado` (divide un rango limpio "8-10" → "4-5"; cualquier otro texto se muestra tal cual) — SOLO lectura: el total se sigue registrando igual, `DetalleEntrenamiento` no cambia. **(b) Ejercicios por tiempo** — `tipo: "tiempo"` reusa `repeticiones` como segundos (sin columna nueva); en el modo sesión, tapear "Serie N" ya no registra al toque: arranca un cronómetro de TRABAJO sobre el mismo panel visual del descanso (`_descanso.html.erb` gana un target `tituloCronometro`/`botonSaltar` reutilizable, "Descanso"/"Saltar" vs. "Trabajo"/"Listo"), y al terminar —solo o cortado antes— registra los **segundos REALES sostenidos** (`Date.now() - inicioTrabajo`), no siempre el objetivo. **(c) Superseries** — `grupo_superserie` (texto libre, dos entradas con el mismo valor arman el par); `sesion_controller.js` alterna el panel entre las dos SIN pasar por `iniciarCronometro` mientras el par no cierre ronda (A hizo su serie → B directo, sin descanso), y solo descansa cuando el SEGUNDO del par completa su serie de esa ronda — el orden lo controla cuál panel está visible, no un estado aparte. **(d) Progresión algorítmica** — `Progresion::Regla`, inline en `DetallesEntrenamientoController#create` (mismo criterio que el detector de PRs: aritmética barata, no HTTP a la IA, sin job). **Decisión de diseño revisada en vivo:** el plan original comparaba contra el TOPE del rango de reps ("doble progresión"), pero el modo sesión SIEMPRE registra el PISO del rango sin pedir que el miembro escriba nada (Fase 18l, fricción cero) — comparar contra el tope habría dejado la regla matemáticamente inalcanzable a través de la UI real (se detectó verificando en navegador, no en los tests unitarios que sí pasaban con reps arbitrarias). Se simplificó a progresión lineal pura (piso completado + mismo peso ya sugerido → sube 2.5kg, propagado a toda aparición del uid vía `Ejercicios::ValidadorRutina.colecciones_de_dias`), tipo StrongLifts, no doble progresión. De paso corrigió un bug latente hermano del de la Nota 18: `_dia_editor.html.erb` también tenía un `Current.user.staff?` sin nil-safety (reventaba en un broadcast Turbo sin `Current.user`) — mismo fix, un local `staff:` enhebrado desde `_semana.html.erb`. **(e) Push del rest-timer** — opt-in APARTE del de racha y apagado por defecto (`users.descanso_push_activo`, a diferencia de `wake_lock_activo`: un push es más intrusivo que la pantalla prendida). `sesion_controller.js` programa el aviso solo si `visibilitychange` detecta que el miembro dejó la pestaña con un cronómetro corriendo, y lo cancela si vuelve antes. **Decisión de diseño revisada en vivo:** el plan original guardaba el `job_id` de Solid Queue para cancelarlo con `SolidQueue::Job#discard`, pero `config.active_job.queue_adapter = :solid_queue` está declarado SOLO en `production.rb` — development y test usan `:async`/`:test` y jamás provisionan `solid_queue_jobs`, así que esa cancelación habría sido inalcanzable (y hasta el propio `perform_later` en el controller reventaba en request specs al intentar `SolidQueue::Job.find_by`). Se cambió a un **token de vigencia** en `users.descanso_push_token`: `NotificarDescansoJob` recibe el token al programarse y es no-op si al ejecutarse ya no coincide con el vigente — cancelar es un simple `UPDATE`, funciona igual en los tres entornos, y es el mismo patrón de idempotencia que `racha_recordada_en` (Fase 15). Verificación de las cinco: recorrido completo en navegador contra datos reales — superserie alternando sin descanso y cerrando ronda, plancha con "Listo" temprano registrando el tiempo real, peso sugerido subiendo tras completar el piso vía chips reales (no fixtures), token de push creándose/cancelándose por HTTP. Cierre: 909 ejemplos / 0 fallos, `dip rubocop` (397 archivos) y `dip brakeman` (0 warnings reales) limpios. **(f) Corrección posterior (septiembre 2026): la regla progresaba UNA sola vez.** `SesionesController#peso_para_registrar` (18l) prefería el kg de la vez pasada al sugerido, y con la regla viva eso armó un lazo: sesión 1 registra 60 (= sugerido) → sube a 62.5; sesión 2 la pantalla muestra "≈62.5" pero registra 60 → la regla ve 62.5 ≠ 60 y no vuelve a subir nunca, y las series subreportan lo levantado (PRs, 1RM, mapa muscular y el Analista §18.7, cuyo criterio de plateau ">3 sesiones sin subir peso" leería un estancamiento falso). La verificación en navegador de (d) cubrió solo el primer incremento. Se descartó `max(anterior, sugerido)`: arregla la subida pero borra las bajadas — el deload del staff (la regla nunca baja sola), la semana de descarga y la fase del ciclo (14.16: la sesión consume números efectivos) quedarían registrados al peso viejo, y la regla volvería a congelarse (sugerido < anterior para siempre). **Decisión: el kg registrado es el sugerido efectivo que la pantalla muestra**; la vez pasada solo rellena si el plan no trae peso. Coherente con §18.7 ("cumplido tal cual" registra el objetivo del plan) y con `Juego::DetectorPr` (el peso del plan confirmado cuenta como peso real). Spec de recorrido completo (GET sesión → POST por chip, dos semanas seguidas) en `spec/controllers/sesiones_controller_spec.rb`. Quedan abiertos: (1) la regla compara contra el sugerido de la BASE, así que en las semanas del mesociclo con `peso_factor` ≠ 1 (Acumulación 1.05, Intensificación 1.1 por defecto) lo registrado nunca coincide y no progresa — tampoco lo hacía antes de esta corrección (resuelto en (g)); (2) sin input de peso ni pantalla de corrección (el DELETE murió en 18n), el miembro que levantó distinto a lo sugerido no puede dejarlo registrado. **(g) Segunda corrección (septiembre 2026): la regla lee la MISMA prescripción que la sesión.** Cierra el abierto (1) de (f). `buscar_entrada` devolvía la primera aparición del uid en `colecciones_de_dias` — la BASE —, así que la regla comparaba contra el sugerido base mientras la sesión registraba el efectivo (semana × fase, 14.16): con el mesociclo por defecto (1.0/1.05/1.1/0.85, el de casi todos los planes IA y de `GeneradorPlanBasico`) solo progresaba en la semana 1, y solo con la fase del ciclo en identidad. Además tomaba series y piso de reps de la base: una semana con `series_delta: 1` subía el peso tras la 3ª serie de 4, y una con `reps_delta` negativo no subía nunca. Se evaluaron dos lecturas — (a) comparar contra lo efectivo con compuertas explícitas, (b) mantener la base y declarar que la ola del mesociclo ES la progresión en las semanas ≠ 1.0 — y Yonatan eligió (a): (b) dependía de una coincidencia aritmética (si la IA emite una semana 1 de 0.9 la regla muere) y dejaba sin progresión el mesociclo entero a una miembra con el ciclo registrado si su día de la semana 1 caía en menstrual o lútea. **Decisión:** `PlanPersonalizado#prescripcion_de(fecha)` es la única resolución de "qué toca esta fecha" — reprogramación (19e: contenido y semana de `fecha_original`), semana del mesociclo (`Rutina::Calendario` con fallback a la semana en curso) y fase del ciclo de la fecha compuesta vía `Rutina::Resolutor.dias(extra:)` —, y la consumen `SesionesController#show` y `Progresion::Regla`. La regla compara peso, número de series y piso de reps contra la entrada EFECTIVA del uid en el día de esa fecha (si el uid no está en ese día no evalúa — ya no se busca en toda la rutina); el incremento sigue propagándose a toda aparición del uid. **Compuertas — un día liviano no es evidencia de progreso:** no progresa en una semana `descarga: true`, ni con una fase del ciclo que recorte carga (`Ciclo::Ajuste.baja_carga?`: menstrual y lútea — por la regla dura de 14.16 la fase solo baja), ni cuando la prescripción efectiva del uid queda por debajo de la base en peso, series o piso de reps (una semana de 0.9 sin marca de descarga, o un recorte manual del staff en una semana materializada). La idempotencia se conserva: tras subir la base, el efectivo del día se recalcula (factor ≥ 0.5 ⇒ al menos +1 kg tras el redondeo a medios) y ya no coincide con lo registrado. **Consecuencia aceptada:** con el mesociclo por defecto la regla sube +2.5 kg por semana en las semanas 1–3, encima de la ola — ≈3 incrementos por mesociclo en vez de 1 (base 60: se levanta 60 / 65.5 / 71.5 y la descarga 57.5); y como el modo sesión registra el piso sin input (abierto (2) de (f)) la compuerta es en la práctica de asistencia: quien tapea sin cumplir sube igual hasta que el staff lo baja. Se descartó limitar a un incremento por mesociclo (estilo 5/3/1): exigía un marcador nuevo en el JSONB y el contrato está cerrado (arriba). Specs de recorrido (GET sesión → POST por chip) a través de las cuatro semanas, semana materializada, fase menstrual, semana liviana sin marca de descarga, `series_delta` y día reprogramado entre semanas, en `spec/controllers/sesiones_controller_spec.rb` (los de las cuatro semanas, materializada y `series_delta` verificados ROJOS contra el código anterior); cada compuerta aislada — descarga al 1.0, lútea compensada por una semana de 1.2, uid fuera del día de la fecha — en `spec/services/progresion/regla_spec.rb`, con mutación de cada una verificada roja. Siguen abiertos (2) de (f) y el `INCREMENTO_KG` fijo, que pesa distinto en un curl de 10 kg que en una sentadilla de 100.
