...
- **Integración a `main`: rebase + fast-forward, sin merge commits** (decisión de Yonatan, 2026-09-23). El ruleset de `main` exige historia lineal; los viejos commits `merge:` de dos padres pasaban solo por el bypass de su cuenta. Flujo: `git fetch origin` → `git rebase origin/main` → suite en verde → `git push origin HEAD:main` (si lo rechaza por non-fast-forward, rebase de nuevo; jamás `--force` sobre `main`). Leer la salida **completa** del push: cualquier «Bypassed rule violations» se reporta, no se filtra.
